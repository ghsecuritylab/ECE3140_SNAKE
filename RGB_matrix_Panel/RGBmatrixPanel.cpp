...
//    init(16,dbuf);
//}
RGBmatrixPanel::RGBmatrixPanel(PinName Pin_R1, PinName Pin_R2,
    ...
    Pin_R1(Pin_R1), Pin_R2(Pin_R2),
    Pin_G1(Pin_G1), Pin_G2(Pin_G2),
    Pin_B1(Pin_B1), Pin_B2(Pin_B2),
    Pin_CLK(Pin_CLK), Pin_LAT(Pin_LAT), Pin_OE(Pin_OE),
    Pin_A(Pin_A), Pin_B(Pin_B), Pin_C(Pin_C), Pin_D(Pin_D),
    _dataBus(Pin_R1,Pin_G1,Pin_B1,Pin_R2,Pin_G2,Pin_B2),
    _rowBus(Pin_A,Pin_B,Pin_C),
    _d(Pin_D),// Init 32x32-specific elements:
    _sclk(Pin_CLK),
    _latch(Pin_LAT),
    _oe(Pin_OE)
{
...
//#ifndef DEBUG
    _refresh.attach_us(callback(this,(&RGBmatrixPanel::updateDisplay)),100);   //updateDisplay() called every 1ms
//#else
...
void RGBmatrixPanel::updateDisplay(void)
{
    ...
    if(++plane >= nPlanes) {        // Advance plane counter.  Maxed out?
        ...
        setPin(Pin_A,  row & 0x01);
        setPin(Pin_B, (row >> 1) & 0x01);
        setPin(Pin_C, (row >> 2) & 0x01);
        setPin(Pin_D, (row >> 3) & 0x01);
    }
    setPin(Pin_OE, 0);
    setPin(Pin_LAT, 0);
    //if(plane > 0) {
...
//    }
    ...
    if(plane>0) {
        for(uint8_t x = 0; x < 32; x++) {
            setPin(Pin_R1, ((*(buffptr+x)>>2)) & 0x01);
            setPin(Pin_G1,((*(buffptr+x)>>3)) & 0x01);
            setPin(Pin_B1,((*(buffptr+x)>>4)) & 0x01);
            setPin(Pin_R2,((*(buffptr+x)>>5)) & 0x01);
            setPin(Pin_G2,((*(buffptr+x)>>6)) & 0x01);
            setPin(Pin_B2,((*(buffptr+x)>>7)) & 0x01);
            setPin(Pin_CLK, 0); // Clock low
            setPin(Pin_CLK, 1); // Clock high
        }
        buffptr += _rawWidth;
    } else {
        for(uint8_t i = 0; i < 32; i++) {
            setPin(Pin_R1, (buffptr[i+64]) & 0x01);
            setPin(Pin_G1,(buffptr[i+64]>>1) & 0x01);
            setPin(Pin_B1,(buffptr[i+32]) & 0x01);
            setPin(Pin_R2,(buffptr[i+32]>>1) & 0x01);
            setPin(Pin_G2,(buffptr[i]) & 0x01);
            setPin(Pin_B2,(buffptr[i]>>1) & 0x01);
            setPin(Pin_CLK, 0); // Clock low
            setPin(Pin_CLK, 1); // Clock high
        }
    }
